<div class="card border-0 shadow-sm mb-3" style="{{ __('lang_dir') == 'rtl' ? 'border-right' : 'border-left' }}: .25rem solid #0ea5e9 !important;">
    <div class="card-body py-3 px-4">
        <p class="mb-0 text-dark">{{ __('This section is used to manage all user workspaces across the application. Review workspace ownership, color labels, creation dates, and linked records, filter the list, open related links, and edit or moderate workspaces when administrative changes are required.') }}</p>
    </div>
</div>
    ...
    <div class="card-header">
        <div class="row g-2 align-items-center">
            ...
            <div class="col-12 col-md-auto">
                <form method="GET" action="{{ route('admin.workspaces') }}" class="d-md-flex admin-filter-form">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-auto">
                                            @if(request()->input('sort'))
                                                <a href="{{ route('admin.workspaces') }}" class="text-secondary">{{ __('Reset') }}</a>
                                            @endif
                                        </div>
                                    </div>
                                </div>
        ...
        @else
            <div class="list-group list-group-flush my-n3">
                                ...
                                <div class="row align-items-center">
                                    <div class="col-12 col-lg-5 d-flex">
                                        <div class="text-truncate">
                                            <div class="d-flex">
                                                <div class="text-truncate">
                                                    <a href="{{ route('admin.workspaces.edit', $workspace->id) }}">{{ $workspace->name }}</a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-12 col-lg-5 d-flex align-items-center">
                                        <div class="d-inline-block {{ (__('lang_dir') == 'rtl' ? 'ml-3' : 'mr-3') }} list-avatar">
                                            <img src="{{ gravatar($workspace->user->email, 48) }}" class="rounded-circle">
                                        </div>
                                        <a href="{{ route('admin.users.edit', $workspace->user->id) }}"@if($workspace->user->trashed()) class="text-danger" @endif>{{ $workspace->user->name }}</a>
                                    </div>

                                    <div class="col-12 col-lg-2 d-flex">
                                        <a href="{{ route('admin.links', ['workspace_id' => $workspace->id]) }}" class="text-dark">{{ $workspace->totalLinks }}</a>
                                    </div>
                                </div>
                            </div>
                            <div class="col-auto">
                                <a href="{{ route('admin.workspaces.edit', $workspace->id) }}" class="btn btn-outline-primary btn-sm">{{ __('Edit') }}</a>
                            </div>
                        </div>
                    </div>
                @endforeach
                ...
                <div class="mt-3 align-items-center">
                    <div class="row">
                        <div class="col">
                            <div class="mt-2 mb-3">{{ __('Showing :from-:to of :total', ['from' => $workspaces->firstItem(), 'to' => $workspaces->lastItem(), 'total' => $workspaces->total()]) }}
                            </div>
                        </div>
                        <div class="col-auto">
